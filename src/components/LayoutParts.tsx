...
export function Footer({ locale }: { locale: Locale }) {
  return <footer className="border-t border-ku-line bg-ku-blue text-white"><div className="container-shell grid gap-8 py-10 md:grid-cols-[1fr_1fr]"><div className="flex gap-4"><LogoMark className="h-16 w-16 rounded-md bg-white p-1" /><div><h2 className="text-2xl font-black">{t[locale].brand}</h2><p className="mt-3 max-w-xl text-white/76">{t[locale].footer.location}</p><p className="mt-2 text-white/76">sports@ku.edu.kw · WhatsApp: +965 0000 0000</p></div></div><div className="grid grid-cols-2 gap-2 text-sm font-semibold md:justify-self-end">{['events','book-facility','files','facilities','team','faq'].map((href) => <Link key={href} href={localizedPath(locale, href)}>{href.replace('-', ' ')}</Link>)}</div></div></footer>;
}
